...
(define (gen-contraposition-rule link-type)
  (let* ((A (Variable "$A"))
         ...
         (precondition (Evaluation
                         (GroundedSchema "py:pln.rules.propositional.gt_zero_confidence")
                         AB))
         (rewrite (ExecutionOutput
                    (GroundedSchema "py:pln.rules.propositional.contraposition_formula")
                    (List
                      NBNA
                      AB
                      A
...
(define (gen-crisp-contraposition-scope-rule scope-link-type)
  (let* ((V (Variable "$V"))
         ...
         (clause PQ)
         (precondition (Evaluation
                         (GroundedPredicate "py:pln.rules.propositional.crisp_contraposition_scope_precondition")
                         PQ))
         (rewrite (ExecutionOutput
                    (GroundedSchema "py:pln.rules.propositional.crisp_contraposition_scope_formula")
                    (List
                      NQNP
                      PQ))))
    (Bind
